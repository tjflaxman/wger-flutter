...
void main() {
  ...
  setUp(() {
    container = ProviderContainer(
      overrides: [gymStateProvider.overrideWith(() => GymStateNotifier())],
    );
  });
  ...
  // No pumpAndSettle() here on purpose: the widget's own Timer.periodic
  // never stops on its own (the workout has no fixed end, unlike the
  // per-exercise rest countdown), so settling would just wait it out until
  // the pumpAndSettle timeout. A single pumpWidget() is enough to read the
  // initial render.
  Future<void> pumpTimer(WidgetTester tester) async {
    await tester.pumpWidget(
      UncontrolledProviderScope(
        container: container,
        child: const MaterialApp(home: Scaffold(body: ElapsedWorkoutTimer())),
      ),
    );
  }
  ...
  testWidgets('Shows zero for a workout start in the future', (tester) async {
    ...
    await withClock(Clock.fixed(now), () async {
      final notifier = container.read(gymStateProvider.notifier);
      notifier.state = GymModeState(workoutStart: now.add(const Duration(minutes: 5)));
