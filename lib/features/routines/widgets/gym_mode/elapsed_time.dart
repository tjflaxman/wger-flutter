...
///
/// Reads the start time from the shared gym state so the value is
/// consistent across the active-workout screen and the session form.
...
class _ElapsedWorkoutTimerState extends ConsumerState<ElapsedWorkoutTimer> {
  // 1Hz ticker, not a long-lived AnimationController -- same reasoning as
  // _RestTimerRow in active_workout_screen.dart: a controller spanning the
  // whole (open-ended) workout would keep widget tests' pumpAndSettle() from
  // ever settling.
  late Timer _uiTimer;
  late DateTime _now;
